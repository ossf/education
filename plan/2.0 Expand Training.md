...
- **M2**: Create job descriptions for contractors for M3
    ...
    - [ ] **M2.1**: Create a job description for secdev contributor per program language
    - [ ] **M2.2**: Get feedback on the job description
...
- **M1**:  Create per standard and/or course an automated action for pulling in updates
    - [ ] **M1.1**: OpenSSF Security Fundamental Course creates CI/CD ingestion action to SKF & SKG
...
# Goal: Create Quizzes & Tests for currently indexed standards (e.g., (m)ASVS & WSTG, PCI-DSS and the course: OpenSSF Security Fundamentals) in the Security Knowledge Graph (SKG).

#### Explanation: Currently, SKF doesn't have any quizzes or test implemented in SKG Database or Web.

Note: Integration with LFX platform

- [ ] **M1**: Design UI for Quizzes & Tests
- [ ] **M2**: Implement Quizes & Tests in SKG Database
- [ ] **M3**: Implement UI for Quizzes & Tests in SKG Web
...
#### Explanation: The current incarnation of the Labs features a full-featured Operating System that runs on the SKF infrastructure. We intend is to pull out and release that Operating System publically, so that Users can install the OS locally and learn with the Labs in their own environment. 
...
## 2.7 Goal: Develop in-person educator-led variations of training materials and deliver bootcamps at conferences 
